...
// ── THE FORK'S OWN PANEL ──────────────────────────────────────────────────────────────────────
//
// Everything above this line is upstream's and arrives with a merge. This section is ours, and it
// is where a fork earns the seam: the panel is the one thing upstream cannot test for us.

console.log("\nThe Kilogent panel:");

const { createKilogentPanel } = await import("../packages/extension/src/providers/kilogent/popup.js");

async function mountKilogent({ store = {}, snapshot = {} } = {}) {
  const window = popupWindow();
  ...
  const section = window.document.createElement("section");
  window.document.getElementById("panels").appendChild(section);
  const panel = createKilogentPanel({ storage, send: rec.send, openTab: (u) => opened.push(u) });
  panel.mount(section);
  await new Promise((r) => setTimeout(r, 0));
  ...
  return { window, section, panel, rec, storage, opened };
}

await check("signed out, it offers sign-in and hides the rest", async () => {
  const { section } = await mountKilogent();
  assert.equal(section.querySelector("#signedOut").classList.contains("hidden"), false);
  assert.equal(section.querySelector("#signedIn").classList.contains("hidden"), true);
  assert.equal(section.querySelector("#signingIn").classList.contains("hidden"), true);
});

await check("signed in, it names who — the whole point of the panel", async () => {
  const { section } = await mountKilogent({
    snapshot: { session: { uid: "u1", email: "a@b.test" }, kilogent: { connState: "connected", tabCount: 2 }, ships: [] },
  });
  assert.equal(section.querySelector("#signedIn").classList.contains("hidden"), false);
  assert.match(section.querySelector("#who").textContent, /a@b\.test/);
  assert.match(section.querySelector("#stateText").textContent, /Connected/);
  assert.match(section.querySelector("#stateText").textContent, /2 tabs in use/);
});

await check("with no email it falls back to the uid rather than saying 'Signed in as '", async () => {
  const { section } = await mountKilogent({ snapshot: { session: { uid: "u1" }, ships: [] } });
  assert.match(section.querySelector("#who").textContent, /u1/);
});

await check("reopening MID-HANDSHAKE shows the code again, not the sign-in button", async () => {
  // The handshake lives in storage because the popup is disposable. Without this read, somebody who
  // closed the popup would start a SECOND login while the first was still pending.
  const { section } = await mountKilogent({
    store: { "kilogent.pending": { userCode: "ABCD1234", expiresAt: Date.now() + 300_000, verificationUrl: "https://x.test/c" } },
  });
  assert.equal(section.querySelector("#signingIn").classList.contains("hidden"), false);
  assert.equal(section.querySelector("#signedOut").classList.contains("hidden"), true);
  assert.equal(section.querySelector("#displayCode").textContent, "ABCD-1234");
  assert.match(section.querySelector("#verifyUrl").textContent, /x\.test/);
});

await check("an EXPIRED handshake does not hold the popup hostage", async () => {
  const { section } = await mountKilogent({
    store: { "kilogent.pending": { userCode: "ABCD1234", expiresAt: Date.now() - 1000 } },
  });
  assert.equal(section.querySelector("#signedOut").classList.contains("hidden"), false, "stuck on a dead code");
});

await check("the blocklist renders, and removing one saves through the worker", async () => {
  const { section, rec } = await mountKilogent({
    store: { "kilogent.blocklist": ["https://bank.test"] },
    snapshot: { session: { uid: "u1" }, ships: [] },
  });
  assert.match(section.querySelector("#blocklist").textContent, /bank\.test/);
  section.querySelector("#blocklist .x").click();
  await new Promise((r) => setTimeout(r, 0));
  const sent = rec.sent.filter((m) => m.type === "setBlocklist").at(-1);
  assert.deepEqual(sent.blocklist, []);
});

await check("a junk blocklist entry is refused and says why", async () => {
  const { section, rec } = await mountKilogent({ snapshot: { session: { uid: "u1" }, ships: [] } });
  section.querySelector("#blockEntry").value = "  ";
  section.querySelector("#blockAdd").click();
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(rec.sent.filter((m) => m.type === "setBlocklist").length, 0, "it saved junk");
  assert.equal(section.querySelector("#error").classList.contains("hidden"), false, "silent refusal");
});

await check("Sign out tells the worker", async () => {
  const { section, rec } = await mountKilogent({ snapshot: { session: { uid: "u1" }, ships: [] } });
  section.querySelector("#signOut").click();
  await new Promise((r) => setTimeout(r, 0));
  assert.ok(rec.sent.some((m) => m.type === "signOut"));
});

await check("it never claims the shell's own messages", async () => {
  // `reconnect` is the shell's button now. A panel that also wired one would fire two.
  const { section } = await mountKilogent({ snapshot: { session: { uid: "u1" }, ships: [] } });
  assert.equal(section.querySelector("#reconnect"), null, "the panel drew its own reconnect button");
});

await check("BOTH panels mount together, and the bridge is folded away", async () => {
  // The arrangement popup.js actually builds here, from the fork's own panels.js.
  const { PANELS } = await import("../packages/extension/src/providers/panels.js");
  const window = popupWindow();
  const deps = { storage: fakeStorage(), send: async () => ({}), openTab: () => {} };
  const panels = PANELS.map((make) => make(deps));
  assert.deepEqual(panels.map((p) => p.name), ["kilogent", "bridge"], "wrong panels, or wrong order");
  assert.equal(panels[0].summary, undefined, "the primary panel was collapsed");
  assert.match(panels[1].summary, /Advanced/, "the bridge panel is not folded away");
  const root = mountAll(window, panels);
  await new Promise((r) => setTimeout(r, 0));
  assert.ok(root.querySelector('section[data-panel="kilogent"] #signedOut'), "kilogent did not mount");
  assert.ok(root.querySelector("details section[data-panel=\"bridge\"] #profiles"), "bridge is not inside a <details>");
});
