...
3. Click the extension → **Sign in with Lumi**. A tab opens at `crew.kilogent.com/connect-browser`
...
and allowing a particular agent to drive it for a particular length of time, both happen in Lumi
...
| **Share** | agents may drive *this* browser | you, in Lumi |
| **Permission** | they may, right now, for this long | you, in Lumi |
...
Chrome shows a permanent **"Lumi Browser is debugging this browser"** strip while a tab is being
...
browser at a bridge you run yourself, with a URL and a shared token. It needs no Lumi account, it
...
```
npm run test:lumi        # the Lumi transport, against a REAL relay — no Chrome needed
npm run test:mock        # the bridge mode's CDP + session model
npm run test:profiles    # multiple bridges, per-profile tab isolation
```

`test:lumi` runs the actual relay from the published `@lumi.ai/relay` package and makes this
