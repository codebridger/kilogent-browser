 ...
 *
 * `getStatus` and `reconnect` are deliberately NOT here. Both need every transport, and a claimed
 * message reaches exactly one — so the worker answers them from `registry.status()` and
 * `registry.reconnectAll()`. Only `saveProfiles` is genuinely this transport's alone.
 */
const MINE = new Set(["saveProfiles"]);
