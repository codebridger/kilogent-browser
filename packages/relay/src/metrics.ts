...
export class RelayMetrics {
   ...
   * the scheduled report to Crew, and an operator or `doctor` curling /v1/health — and if both
   * closed the window they would steal each other's: a curl during the 30-second gap would take
   * the interval Crew was about to be told about, and Crew would be sent a near-empty one. The
   * periodic reporter is the single writer; every other reader looks without touching.
   */
