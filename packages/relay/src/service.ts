...
//
// Ported from `packages/crew/runner/src/service.ts` — hand-written unit files
...
export function systemdUnit(): string {
  return `[Unit]
Description=${RELAY_NAME} — browser relay
...
export function installService(): InstallResult {
  ...
  if (!fs.existsSync(envFile())) {
    throw new ServiceError(
      `No configuration at ${envFile()}. Run \`${RELAY_NAME} setup\` first — the unit points at that ` +
