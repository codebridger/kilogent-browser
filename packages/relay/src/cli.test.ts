...
describe("setup", () => {
  beforeAll(() => fs.rmSync(envFile(), { force: true }));
  ...
  it("KEEPS the keys when run again — rotating silently would disconnect the fleet", () => {
    // The ticket key is shared with whatever mints credentials. Rotating it refuses every
    // connected browser with
...
describe("the update ladder on a root-owned prefix", () => {
  it("refuses with two commands, not `sudo lumi-relay update`", async () => {
    ...
    //
    // The refusal must NOT say "run it with sudo", which is the obvious shortcut and is wrong:
    // the restart goes through `systemctl --user`, and under sudo that addresses ROOT's user
    // manager rather than the account the relay runs as. The install would succeed and the
    // running process would never be replaced — the exact silent no-op the whole ladder exists
    // to prevent.
...
describe("ticket", () => {
  const env = {
    RELAY_CONTROL_KEY: "c".repeat(64),
    RELAY_TICKET_KEY: "t".repeat(64),
  } as NodeJS.ProcessEnv;

  it("mints a credential the configured provider will accept", async () => {
    const { resolveAuthProvider } = await import("./providers/index.js");
    const plan = planTicket(env, { owner: "me", browser: "mac" }, 1_000);
    assert.equal(plan.ok, true);
    // The round trip is the assertion. A mint that produced a well-formed string the relay
    // refuses is exactly the failure this command exists to make impossible to ship.
    const verdict = resolveAuthProvider(env).verifyBrowser(plan.ok && plan.credential, 2_000);
    assert.equal(verdict.ok, true);
    assert.equal(verdict.ok && verdict.identity.browserId, "mac");
  });

  it("takes --ttl in MINUTES, not milliseconds", async () => {
    // The unit is the whole risk of this flag: `--ttl 2` meaning two milliseconds would produce a
    // credential that is expired by the time it reaches a browser, and look like a broken relay.
    const { resolveAuthProvider } = await import("./providers/index.js");
    const plan = planTicket(env, { owner: "me", browser: "mac", ttl: "2" }, 0);
    assert.equal(plan.ok, true);
    const verdict = resolveAuthProvider(env).verifyBrowser(plan.ok && plan.credential, 0);
    assert.equal(verdict.ok && verdict.identity.expiresAt, 120_000);
  });

  it("refuses a ttl that is not a positive number, rather than minting a NaN expiry", () => {
    for (const ttl of ["0", "-5", "soon", ""]) {
      const plan = planTicket(env, { owner: "me", browser: "mac", ttl });
      assert.equal(plan.ok, false, `expected refusal for ttl ${JSON.stringify(ttl)}`);
      assert.match(plan.ok === false ? plan.message : "", /--ttl must be a positive number/);
    }
  });

  it("passes the provider's own refusal through instead of inventing one", () => {
    const plan = planTicket(env, { owner: "a:b", browser: "mac" });
    assert.equal(plan.ok, false);
    assert.match(plan.ok === false ? plan.message : "", /invalid ownerId/);
  });

  it("says a provider cannot mint BY NAME, so it does not read like a breakage", () => {
    const plan = planTicket(
      { RELAY_AUTH: "token", RELAY_CONTROL_KEY: "c".repeat(64), RELAY_TOKENS: `mac=${"a".repeat(32)}` },
      { owner: "me", browser: "mac" }
    );
    assert.equal(plan.ok, false);
    assert.match(plan.ok === false ? plan.message : "", /`token` provider does not mint/);
  });

  it("reports a misconfigured provider instead of throwing out of the command", () => {
    const plan = planTicket({ RELAY_CONTROL_KEY: "c".repeat(64) }, { owner: "me", browser: "mac" });
    assert.equal(plan.ok, false);
    assert.match(plan.ok === false ? plan.message : "", /RELAY_TICKET_KEY/);
  });
});
...
describe("masking a secret for display", () => {
  ...
  it("masks EVERY credential variable, not the ones whose name ends in KEY", () => {
    // The old rule was `/KEY$/`, which was a rule about the two variables that existed when it was
    // written. `RELAY_TOKENS` arrived and `config list` — the command documented as safe to run
    // while screen-sharing — printed a browser's credential in full.
    for (const name of ["RELAY_CONTROL_KEY", "RELAY_TICKET_KEY", "RELAY_TOKEN", "RELAY_TOKENS"]) {
      assert.equal(isSecretVar(name), true, `${name} is not treated as a secret`);
    }
    assert.equal(maskConfigValue("RELAY_TOKEN", SECRET).includes(SECRET), false);
    assert.equal(maskConfigValue("RELAY_TOKENS", `laptop=${SECRET}`).includes(SECRET), false);
  });

  it("keeps the machine NAMES in RELAY_TOKENS — they are the ids a dispatch is addressed to", () => {
    const shown = maskConfigValue("RELAY_TOKENS", `laptop=${SECRET},desktop=${"d".repeat(32)}`);
    assert.match(shown, /laptop=/);
    assert.match(shown, /desktop=/);
    assert.equal(shown.includes(SECRET), false);
    assert.equal(shown.includes("d".repeat(32)), false);
  });

  it("masks an unparseable RELAY_TOKENS whole rather than guessing at its shape", () => {
    const shown = maskConfigValue("RELAY_TOKENS", SECRET);
    assert.equal(shown.includes(SECRET), false);
  });

  it("hides a SHORT secret entirely instead of printing most of it", () => {
    // `slice(0,6)…slice(-4)` on an eight-character value shows all eight.
    assert.equal(maskConfigValue("RELAY_TOKEN", "hunter22").includes("hunter"), false);
  });

  it("leaves everything that is not a credential alone", () => {
    assert.equal(isSecretVar("RELAY_PORT"), false);
    assert.equal(maskConfigValue("RELAY_PORT", "8787"), "8787");
    assert.equal(maskConfigValue("RELAY_HEALTH_URL", "https://x.test/h"), "https://x.test/h");
    // Named like a secret, is not one. The list is exact, not a pattern.
    assert.equal(isSecretVar("RELAY_INSTANCE_ID"), false);
  });
});
