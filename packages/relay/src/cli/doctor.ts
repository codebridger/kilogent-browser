...
function configFileChecks(): { checks: Check[]; config: RelayConfig | null } {
  ...
  try {
    ...
    checks.push({
      ...
      fix: `Run \`${RELAY_NAME} setup\` (it never rotates an existing key unless you pass --rotate).`,
    });
  }
...
function serviceChecks(): Check[] {
    ...
    return [
      {
        ...
        fix: `Run \`${RELAY_NAME} service install\`.`,
      },
    ];
  }
  ...
  checks.push({
    ...
    detail: status.detail,
    ...(status.state === "running" ? {} : { fix: `Run \`${RELAY_NAME} service start\` and check \`${RELAY_NAME} logs\`.` }),
  });
  ...
  if (status.execMissing) {
    checks.push({
      ...
      fix: `Run \`${RELAY_NAME} service install\` again from the install you want it to use.`,
    });
  }
  ...
  if (process.platform === "linux" && status.unitPath) {
    ...
    if (unit && !unit.includes("LimitNOFILE=")) {
      checks.push({
        ...
        fix: `Re-run \`${RELAY_NAME} service install\` to rewrite the unit.`,
      });
    }
...
async function reachabilityChecks(config: RelayConfig | null): Promise<Check[]> {
    ...
    return [
      {
        ...
        fix: `Is it running? \`${RELAY_NAME} service status\`, then \`${RELAY_NAME} logs\`.`,
      },
    ];
  }
  if (!liveness.ok) {
  ...
  // The authed probe is the one that proves the caller's own request would work — same key,
  // same route.
  ...
  try {
    ...
    if (res.status === 401) {
      checks.push({
        ...
        fix: `It is running with an older configuration. \`${RELAY_NAME} service restart\`.`,
      });
      return checks;
    }
    if (!res.ok) {
  ...
  if (version && version !== RELAY_VERSION) {
    checks.push({
      ...
      fix: `\`${RELAY_NAME} service restart\` picks up the installed version.`,
    });
  }
    ...
    checks.push(
      maxFds < NOFILE_LIMIT
        ? {
            ...
            fix: `\`${RELAY_NAME} service install\` then \`${RELAY_NAME} service restart\`.`,
          }
        : used > FD_WARN_AT
          ? {
              name: "fd headroom",
