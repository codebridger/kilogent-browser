...
export function planUpdate(facts: UpdateFacts): UpdateDecision {
  if (detectInstallMode(facts.runningCli) === "checkout") {
    return {
      ...
      fix: `git pull && npm ci && npm run build -w packages/relay && ${RELAY_NAME} service restart`,
    };
  }
  ...
  if (facts.serviceInstalled && facts.serviceExec) {
    if (detectInstallMode(facts.serviceExec) !== "global-npm") {
      return {
        ...
        fix: `Run \`${RELAY_NAME} service install\` from the install you want it to use, then update.`,
      };
    }
  }
...
/**
 * Can this process write the global install directory?
 *
 * Found by running `update` on the real box rather than by reading the code: a relay installed on
 * a machine whose node came from the system package manager has an npm prefix of `/usr`, so
 * `npm i -g` needs root and fails with EACCES buried in twenty lines of npm advice.
 *
 * Checked BEFORE npm runs, so the refusal is one clear sentence instead of a wall — the same
 * fail-closed-with-a-reason posture the rest of this ladder has.
 */
function globalPrefixWritable(): { ok: boolean; dir: string } {
  const prefix = spawnSync("npm", ["config", "get", "prefix"], { encoding: "utf8" });
  const dir = path.join(`${prefix.stdout ?? ""}`.trim() || "/usr/local", "lib", "node_modules");
  try {
...
export interface UpdateOutcome {
  status: "refused" | "unchanged" | "updated" | "install-failed" | "unverified" | "needs-root";
  message: string;
  from?: string;
  to?: string;
}
...
export function runUpdate(options: { channel?: string; restart?: boolean } = {}): UpdateOutcome {
  ...
  const writable = globalPrefixWritable();
  if (!writable.ok) {
    return {
      status: "needs-root",
      message:
        ...
        `Run them separately. \`sudo ${RELAY_NAME} update\` would install correctly and then restart\n` +
        "the wrong thing: the restart is `systemctl --user`, which under sudo targets root's user\n" +
        "manager rather than the account this relay actually runs as.",
    };
  }
