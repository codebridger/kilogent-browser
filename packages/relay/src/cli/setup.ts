// `setup` — write `relay.env`, minting the two keys if they do not exist yet.
//
// IDEMPOTENT, AND THAT IS A SAFETY PROPERTY RATHER THAN A CONVENIENCE. Re-running setup must never
// rotate a key: the ticket key is shared with whatever mints credentials, and rotating it silently
// disconnects every browser on the fleet with `unauthorized` — which the extension correctly
// treats as "stop and tell the human" rather than "retry". Rotation is `--rotate`, spelled out
// loud, and it prints what has to change on the other side before it will help.
