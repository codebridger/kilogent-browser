...
import { browserKey, mintRelayTicket, verifyRelayTicket, TICKET_TTL_MS } from "./ticket.js";
...
describe("relay tickets", () => {
  it("round-trips the identity it was minted with", () => {
    const t = mintRelayTicket(KEY, { ownerUid: "uid_navid", browserId: "brw_mac" }, 1_000);
  ...
  it("refuses a ticket signed with a different key", () => {
    const t = mintRelayTicket(OTHER, { ownerUid: "u", browserId: "b" });
    assert.deepEqual(verifyRelayTicket(KEY, t), { ok: false, reason: "unauthorized" });
  });

  it("refuses a tampered payload", () => {
    // The whole point of the signature: swapping the uid must not survive it.
    const t = mintRelayTicket(KEY, { ownerUid: "uid_navid", browserId: "b" });
    const [, sig] = t.split(".");
  ...
  it("tells an EXPIRED ticket apart from a bad one", () => {
    ...
    const t = mintRelayTicket(KEY, { ownerUid: "u", browserId: "b", ttlMs: 1_000 }, 0);
  ...
  it("refuses a version it does not understand, rather than guessing", () => {
    const payload = Buffer.from(
      JSON.stringify({ v: 99, u: "u", b: "b", exp: Date.now() + 60_000 })
    ).toString("base64url");
    // Signed with the REAL key, so only the version check can be what refuses it.
    const sig = createHmac("sha256", KEY).update(payload).digest().toString("base64url");
    assert.deepEqual(verifyRelayTicket(KEY, `${payload}.${sig}`), {
      ok: false,
      reason: "unauthorized",
    });
  });

  it("refuses an identity that could name two browsers with one key", () => {
    // `${uid}:${browserId}` is the hub key, so a colon inside either half is an ambiguity.
    // Unreachable through Crew today; refused anyway so it is unrepresentable rather than merely
    // unlikely.
    const t = mintRelayTicket(KEY, { ownerUid: "a", browserId: "b:c" });
    assert.deepEqual(verifyRelayTicket(KEY, t), { ok: false, reason: "unauthorized" });
    const t2 = mintRelayTicket(KEY, { ownerUid: "a:b", browserId: "c" });
    assert.deepEqual(verifyRelayTicket(KEY, t2), { ok: false, reason: "unauthorized" });
  });
  ...
  it("refuses everything when the relay has no key configured", () => {
    const t = mintRelayTicket(KEY, { ownerUid: "u", browserId: "b" });
