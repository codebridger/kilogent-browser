// Wire protocol between the relay (our VM) and the Lumi Browser extension.
//
...
//
// WHY THIS IS A SEPARATE FILE FROM `packages/bridge-server/src/protocol.ts`, which it resembles:
// they speak to two different modes of the same extension. The bridge authenticates a static
// shared token typed into a form by whoever runs it, and serves an MCP face itself; the relay
// authenticates a short-lived ticket minted by Lumi Crew and serves no MCP at all. They will
// diverge further (Ship rows, blocklist pushes, browser identity), and coupling them would mean
// every change made for Lumi risks the self-hosted path that is both our dev loop and the reason
// the repo is worth open-sourcing. What IS deliberately identical is the command vocabulary —
// `cmd`/`res`/`status`/`ping` — so `src/executor.js` never learns which server it is talking to.
 ...
 *
 * IDENTITY COMES FROM THE TICKET AND NOTHING ELSE. There is deliberately no `ownerUid` or
 * `browserId` field here: a self-asserted id beside a signed one is an invitation to check the
...
export interface HelloMsg {
  t: "hello";
  /** Protocol version. 2 is the Lumi ticket protocol; 1 was the bridge's shared-token hello. */
  v: 2;
  /** Opaque ticket minted by Crew's `mintBrowserRelayTicket`. See ticket.ts. */
  ticket: string;
...
/** Auth accepted. Echoes the identity the ticket proved, so the popup can show whose it is. */
...
export interface CmdMsg {
  ...
  deadlineMs: number;
  /** The tab group this call belongs to — one Crew job. Absent means an unscoped one-off. */
  sessionId?: string;
}

/**
 * Open a tab group, with the Ship's blocklist attached.
 *
 ...
 * single action. See `session_config` for what happens when a captain edits it mid-run.
 */
