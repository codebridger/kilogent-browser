// Telling the caller how the box is doing, every 30 seconds.
//
// PUSH, NOT PULL, and the reason is the tunnel. The caller is typically serverless, with no fixed
...
//
// It authenticates with the CONTROL key rather than with an identity, because a relay is not a
// principal in the caller's system — it holds no credential for it by design. `RELAY_HEALTH_URL`
// must therefore be https (config.ts enforces it): the key travels in that header.
...
export class HealthReporter {
  ...
  private async report(): Promise<void> {
    ...
    try {
      ...
      // Log the first failure and then back off logarithmically. A relay that cannot report
